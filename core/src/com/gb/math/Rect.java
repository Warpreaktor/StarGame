...
public class Rect {
    ...
    public final Vector2 pos = new Vector2(); // текущая позиция на экране, от центра объекта.
    ...
    /**
     *Провеяем, попадает ли переданный вектор в this.Rect.
     * Т.е. в зону обозначенную прямоугольником.
     */
    public boolean isMe(Vector2 touch) {
        return touch.x >= getLeft() && touch.x <= getRight() && touch.y >= getBottom() && touch.y <= getTop();
    }
    /**
     *Провеяем пересечение двух прямоугольников.
     */
